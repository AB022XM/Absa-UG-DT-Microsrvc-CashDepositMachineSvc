...
import ug.co.absa.microsrvc.cdmsvc.IntegrationTest;
...
class AbsaUgdtMicroCashDepositMachineSvcKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/absa-ugdt-micro-cash-deposit-machine-svc-kafka/publish?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/absa-ugdt-micro-cash-deposit-machine-svc-kafka/register"))
            .andExpect(status().isOk())
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/absa-ugdt-micro-cash-deposit-machine-svc-kafka/unregister"));
                return;
            }
        }
